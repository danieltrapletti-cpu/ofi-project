/* pensieri-miei.js — OFI (paginazione + commenti/reazioni/Lumina + video poster + Storia-Emozioni) */
...
// ➜ Storia-Emozioni (solo calcolo/DOM, NO write)
...
function renderPostCard(p){
  ...
  qs(".toggle-vis-min", el)?.addEventListener("click", async ()=>{
    ...
    const reactsRow = qs(".reactions-row", el);
    const shareBtn  = qs(".sharePost", el);
    const commBtn   = qs(".openComments", el);

    if (next === "pubblico"){
      reactsRow && (reactsRow.style.display = "");
