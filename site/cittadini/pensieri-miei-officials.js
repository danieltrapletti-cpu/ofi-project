...
function ensureEmotionActionStyles(){
  ...
  /* Chip "Aggiungi riflessione" quando non c'è card */
  .emo-add-chip{
  ...
  @media (max-width:640px){
    ...
    .emo-add-chip{ font-size:.85rem; padding:.3rem .5rem; top:8px; right:8px; }
  }`;
...
/* Chip “Aggiungi riflessione” quando non esiste la card */
function injectAddEmotionChip(hostBox, postId){
  ensureEmotionActionStyles();
  if (!hostBox || hostBox.querySelector('.emo-add-chip')) return;
  if ((hostBox.dataset.owner||'') !== (auth.currentUser?.uid||'')) return;

  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'emo-add-chip';
...
async function refreshEmotionUI(postId, hostBox){
  try{
    ...
    if (inside){
      ...
      // se non c'è nessuna card → mostra il chip "Aggiungi riflessione"
      injectAddEmotionChip(hostBox, postId);
    }
  }catch{
    // in caso di errore, almeno permetti di aggiungere
    injectAddEmotionChip(hostBox, postId);
  }
}
    ...
    if (isPriv && !hasEmo){
      openEmotionQuickDialog({
        ...
        async onSave({ text, makePublic }){
          ...
          const box = document.querySelector(`.pensiero-box[data-pid="${pid}"]`);
          if (box){ try { await refreshEmotionUIRetry(pid, box); } catch {} }
        }
      });
    } else {
      removeQueryParam('newPostId');
