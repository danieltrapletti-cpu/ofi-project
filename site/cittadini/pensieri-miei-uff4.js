...
// ⬇️ PATCH 1: aggiungi questa riga QUI
...
// Notifica istantanea al DOM: aggiorna/crea card senza refresh
...
/* Chip “Aggiungi riflessione” — inserito nella testata per evitare overlap con Lumina */
...
// 3) aspetta un attimo che updatedAt/serverTimestamp diventi “visibile” nelle query
await sleep(950);

// 4) refresh robusto: prima query diretta, poi UI-safe
...
await refreshEmotionUIRetry(postId, hostBox);
    ...
    }); // <-- chiusura CORRETTA di openEmotionQuickDialog
  });   // <-- chiusura CORRETTA dell'addEventListener
}
...
async function refreshEmotionUI(postId, hostBox){
  try{
    ...
    const above = post.previousElementSibling?.classList?.contains('ofi-emozione') ? post.previousElementSibling : null;
    ...
    if (ok && inserted){
      // 3) sposta sopra il post e aggancia azioni
      ...
      // 4) SOLO ORA rimuovi eventuali vecchie (optimistic/preview)
      if (above && above !== inserted) above.remove();
      oldsInside.forEach(n => { if (n !== inserted) n.remove(); });
    } else {
      // Nessun dato ancora: lascia eventuale preview, altrimenti mostra chip
      if (!above && oldsInside.length === 0){
...
function showEmotionOptimistic(postId, hostBox, text, scope='public'){
  ...
  // pulizia: rimuovi eventuale card emozione subito sopra
  const prev = post.previousElementSibling;
  ...
  // inserisci sopra al post e aggancia le azioni (kebab) se autore
...
async function maybePromptRiflessioneOnNewPost(){
  ...
  try {
    ...
    if (isPriv && !hasEmo){
      ...
      // nulla da chiedere, rimuovi il param
      removeQueryParam("newPostId");
    }
  } catch(e){
    // in qualunque errore, pulisci il parametro query
    removeQueryParam("newPostId");
  }
}
...
async function renderPostCard(p){
  ...
  el.querySelector(".toggle-vis-min")?.addEventListener("click", async ()=>{
    ...
    // aggiorna subito i contatori di testata reali
    refreshKpiFromServer().catch(()=>{});
  });
