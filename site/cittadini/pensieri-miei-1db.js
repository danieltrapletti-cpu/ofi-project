...
async function fetchLatestEmotionDoc(postId){
  ...
  // tentativo "ricco" (può richiedere indice): state==live + orderBy(updatedAt desc)
  try{
    ...
    // se manca l'indice, ignoriamo e passiamo al fallback
    console.debug('fetchLatestEmotionDoc: fallback senza indice', e?.code || e);
  }

  // fallback 1: solo ordine per updatedAt
  try{
  ...
  // fallback 2: nessun ordine (ultimo preso "a caso", ma sufficiente per edit/delete)
  try{
...
async function deleteEmotionForPost(postId){
  ...
  // svuota preview (se permesso dalle regole)
  try{
...
async function editEmotionForPost(postId, { text, makePublic }) {
  ...
  // cerca l'ultima emozione "live" — se non c’è, creeremo
  let current = null;
  try {
  ...
  // aggiorna la preview sul doc principale (fallback UI)
  try {
    await updateDoc(doc(db, 'pensieri_utente', postId), {
...
/* ===== Bottoni azione sulla card Emozione ===== */
function ensureEmotionActionStyles(){
  ...
  .ofi-emo-actions{
    ...
    border: 1px solid rgba(0,0,0,.1); border-radius: 10px; padding: 4px 6px;
    box-shadow: 0 6px 18px rgba(0,0,0,.12);
  }
  [data-theme="dark"] .ofi-emo-actions{
    background: rgba(16,20,32,.7); border-color: rgba(255,255,255,.14);
  }
  .ofi-emo-actions .emo-btn{
    border:0; background:transparent; font-size:.95rem; line-height:1;
    padding:.35rem .55rem; border-radius:8px; cursor:pointer; opacity:.9;
  }
  .ofi-emo-actions .emo-btn:hover{ opacity:1; transform: translateY(-1px); }

  /* ——— Mobile: barra più piccola e discreta ——— */
  @media (max-width: 640px){
    .ofi-emo-actions{
      top:6px; right:6px; gap:4px; padding: 2px 4px;
      border-radius: 9px; box-shadow: 0 6px 14px rgba(0,0,0,.18);
    }
    .ofi-emo-actions .emo-btn{
      font-size:.85rem; padding:.3rem .45rem;
    }
    /* opzionale: leggera trasparenza extra */
    .ofi-emo-actions{ background: rgba(16,20,32,.55); }
    .ofi-emo-actions .emo-btn{ color:#fff; opacity:.85; }
  }
  `;
...
// Monta i bottoni Modifica/Elimina (solo autore)
function attachEmotionActions(hostBox, postId, emoEl){
  ...
  if (!emo) return;
  if (emo.querySelector('.ofi-emo-actions')) return; // evita duplicati

  const isOwner = (hostBox?.dataset?.owner || '') === (auth.currentUser?.uid || '');
  if (!isOwner) return;

  const bar = document.createElement('div');
  bar.className = 'ofi-emo-actions';
  bar.innerHTML = `
    <button type="button" class="emo-btn" data-act="edit" title="Modifica">✎</button>
    <button type="button" class="emo-btn" data-act="del"  title="Elimina">🗑</button>`;
  emo.appendChild(bar);

  // EDIT (crea se manca)
  bar.querySelector('[data-act="edit"]').addEventListener('click', async ()=>{
    ...
    openEmotionQuickDialog({
      ...
      async onSave({text, makePublic}){
        ...
        if (!current && newText){
          const emoCol = collection(db, `pensieri_utente/${postId}/emozioni_cronaca`);
          await addDoc(emoCol, {
            text: newText,
            scope: makePublic ? 'public' : 'private',
            state: 'live',
            uid: (auth?.currentUser?.uid||''),
            createdAt: serverTimestamp(),
            updatedAt: serverTimestamp()
          ...
          await updateDoc(doc(db,'pensieri_utente', postId), {
            riflessione_preview: newText.slice(0,240),
            updatedAt: serverTimestamp()
          });
        } else {
  ...
  bar.querySelector('[data-act="del"]').addEventListener('click', async ()=>{
    ...
    const ok = await deleteEmotionForPost(postId);
    if (!ok){
      await updateDoc(doc(db,'pensieri_utente', postId), {
        riflessione_preview: '',
        updatedAt: serverTimestamp()
...
async function refreshEmotionUI(postId, hostBox){
  try{
    ...
    // rimuovi eventuale card precedente (sia sopra che dentro)
    if (post.previousElementSibling?.classList?.contains('ofi-emozione')){
      post.previousElementSibling.remove();
    }
    hostBox?.querySelectorAll('.ofi-emozione').forEach(n=>n.remove());

    // reinserisci dentro al box…
    await maybeInsertEmotion({ db, postId, into: hostBox, opts:{ showAlsoPrivate:true } });

    // …poi sposta la card appena inserita prima del post
    const inside = hostBox.querySelector('.ofi-emozione');
    if (inside){
      inside.classList.add('feed-card');
      post.parentElement.insertBefore(inside, post);
      attachEmotionActions(hostBox, postId, inside);
    }
  }catch{}
}
...
async function onPostBecamePublicFirstTime(pid){
  try{
    if (wasEmotionAsked(pid)) return; // già chiesto
    ...
    openEmotionQuickDialog({
      ...
      async onSave({ text, makePublic }){
        ...
        try {
          ...
          if (box) await refreshEmotionUI(pid, box);
...
async function maybePromptRiflessioneOnNewPost(){
  ...
  try{
    ...
    if (isPriv && !hasEmo){
      openEmotionQuickDialog({
        onSkip(){ removeQueryParam('newPostId'); },
        async onSave({ text, makePublic }){
          ...
          await updateDoc(doc(db,'pensieri_utente', pid), {
            riflessione_preview: text.slice(0,240),
            updatedAt: serverTimestamp()
          });
          const box = document.querySelector(`.pensiero-box[data-pid="${pid}"]`);
          if (box){ try { await refreshEmotionUI(pid, box); } catch {} }
        }
      });
    } else {
      removeQueryParam('newPostId');
...
async function renderPostCard(p){
  ...
  // Toggle visibilità (pubblico/privato)
  ...
  for (const p of visiblePosts){
    ...
    try {
      ...
      if (emo) {
        ...
        // Fallback: usa la preview salvata nel doc principale
      ...
      // In caso di errore, prova comunque il fallback preview
