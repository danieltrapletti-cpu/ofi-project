/* pensieri-miei.js — OFI (paginazione + commenti/reazioni/Lumina) */
...
function renderPostCard(p){
  ...
  el.innerHTML = `
    <div class="pensiero-box lv-${p.lumina_level||0}" data-pid="${p.id}" data-owner="${p.uid||''}" data-vis="${isPub?'pubblico':'privato'}">
      ...
      ${p.media ? `<div class="media"><img alt="" src="${p.media}"></div>` : ``}
      ...
      ${isPub ? `
        <div class="reactions-row">
          <button class="react-btn" data-type="cuore"><span class="emo">❤️</span> <span class="count">${p.reazioni?.cuore||0}</span></button>
          <button class="react-btn" data-type="luce"><span class="emo">✨</span> <span class="count">${p.reazioni?.luce||0}</span></button>
          <button class="react-btn" data-type="colomba"><span class="emo">🕊️</span> <span class="count">${p.reazioni?.colomba||0}</span></button>
          <button class="react-btn" data-type="rosa"><span class="emo">🌹</span> <span class="count">${p.reazioni?.rosa||0}</span></button>
        </div>` : ``}
      <div class="meta"><span>${luogo}</span><div class="hashtag-row">${hashtags}</div><div class="small">${fmtDate(dt)}</div></div>
      <div class="actions-row">
        ${isPub
          ? `<button class="btn primary openComments">Commenti</button>`
          : `<button class="btn openComments onlyIfHasComments" style="display:none">Commenti (solo tu)</button>`
        }
  ...
  const img = qs(".media img", el);
  if(img){ img.addEventListener("load", ()=> smartMediaFit(img, qs(".media", el))); }
...
async function hookReactions(root, post){
  ...
  if(my){ qs(`.react-btn[data-type="${my}"]`, root)?.classList.add("active"); }
  qsa(".react-btn", root).forEach(btn=>{
    btn.addEventListener("click", async ()=>{
      ...
      qsa(".react-btn", root).forEach(b=>b.classList.toggle("active", b.dataset.type===my));
      const c = {
        cuore: +qs('.react-btn[data-type="cuore"] .count', root).textContent,
        luce:  +qs('.react-btn[data-type="luce"]  .count', root).textContent,
        colomba:+qs('.react-btn[data-type="colomba"] .count', root).textContent,
        rosa:  +qs('.react-btn[data-type="rosa"]  .count', root).textContent
      };
      if(prev){ c[prev] = Math.max(0, c[prev]-1); }
      if(my){ c[my] = (c[my]||0)+1; }
      for(const k of Object.keys(c)){ qs(`.react-btn[data-type="${k}"] .count`, root).textContent = c[k]; }
      try{
...
/* ===============================
   OFI — Media Viewer v3 + Fix video desktop
   =============================== */

// ---- 1) Fix: trasforma img->video se l'URL è un video (anche fuori .media)
...
// ---- 2) Media Viewer: preferisci <video>, mostra SOLO reazioni + commenti
...
function openMediaViewer(fromPost){
  ...
  // media: prima video, poi immagine
  const vid = q(fromPost, 'video') || q(fromPost, '.media video');
  const img = q(fromPost, 'img')   || q(fromPost, '.media img');

  let node = null;
  if (vid){
    const src = vid.currentSrc || vid.src || (q(vid, 'source')?.src || '');
    ...
    node.alt = img.alt || 'Immagine';
  } else {
    return;
  }
  mvContent.appendChild(node);

  // Reazioni: whitelist per emoji (cuore, scintilla, colomba, rosa)
  // Cerco bottoni con emoji in uno span.emo o nel textContent
  const EMO_SET = ['❤️','✨','🕊','🌹'];
  function findReactionButtons(scope){
    ...
    return btns.filter(b=>{
      ...
      return EMO_SET.includes(emo);
    }).map(b=>{
      const emo = (b.querySelector('.emo')?.textContent || b.textContent || '').trim();
      const cntEl = b.querySelector('.cnt');
      return { btn:b, emo, cntEl };
    });
  }
  ...
  if (postReacts.length){
    ...
    postReacts.forEach(({btn, emo})=>{
      const mirror = btn.cloneNode(true);
      mirror.removeAttribute('id');
      // inoltra click al bottone originale che ha la stessa emoji
      mirror.addEventListener('click', ()=>{
        const target = findReactionButtons(fromPost).find(x=>x.emo===emo)?.btn;
        if (target) target.dispatchEvent(new MouseEvent('click', {bubbles:true}));
        // aggiorna contatore dopo piccolo delay
        setTimeout(()=>{
          const fresh = findReactionButtons(fromPost).find(x=>x.emo===emo)?.cntEl?.textContent;
          const mine  = mirror.querySelector('.cnt');
  ...
  // Commenti: clona SOLO il bottone che apre i commenti
  let origCmnt = fromPost.querySelector('[data-toggle-comments]');
  if (!origCmnt){
    // fallback: bottone con testo "Commenti"
    origCmnt = Array.from(fromPost.querySelectorAll('button, [role="button"]'))
      .find(b => /comment/i.test(b.textContent || ''));
  }
  if (origCmnt){
