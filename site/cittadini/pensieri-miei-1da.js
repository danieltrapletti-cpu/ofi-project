...
// ===== CRUD emozione più recente del post (Storia dell’autore) =====
...
async function editEmotionForPost(postId, {text, makePublic}){
  ...
  await updateDoc(emo._ref, patch);
  // salviamo una preview per render veloce nelle liste
  await updateDoc(doc(db,'pensieri_utente', postId), { riflessione_preview: patch.text.slice(0,240), updatedAt: serverTimestamp() });
  return true;
}
...
async function deleteEmotionForPost(postId){
  const emo = await fetchLatestEmotionDoc(postId);
  if (!emo) return false;
  await deleteDoc(emo._ref);
  // pulizia preview
  await updateDoc(doc(db,'pensieri_utente', postId), { riflessione_preview: '', updatedAt: serverTimestamp() });
  return true;
}

// Reinserisce la card emozione sopra al post
async function refreshEmotionUI(postId, hostBox){
  // rimuovi la/e card corrente/i
  const parent = hostBox?.parentElement;
  parent?.querySelectorAll('.ofi-emozione.feed-card').forEach(n=>n.remove());
  // reinserisci
  try{
    await maybeInsertEmotion({ db, postId, into: hostBox, opts:{ showAlsoPrivate:true } });
    const emo = parent?.querySelector('.ofi-emozione.feed-card');
    if (emo) attachEmotionActions(hostBox, postId);
  }catch{}
}

// Monta bottoni Modifica/Elimina sulla card emozione (solo autore)
function attachEmotionActions(hostBox, postId){
  const emo = hostBox?.parentElement?.querySelector('.ofi-emozione.feed-card');
  if (!emo) return;
  if (emo.querySelector('.ofi-emo-actions')) return; // evita duplicati

  // sicurezza: solo autore del post
  ...
  bar.querySelector('[data-act="edit"]').addEventListener('click', async ()=>{
    // pre-carica testo attuale e apre il dialog
    ...
    // pre-riempie il dialog dopo il mount
    setTimeout(()=>{
...
function ensurePrivateUI(box, p){
  ...
  // 7) BONIFICA eventuali cloni nel Media Viewer
  try {
...
async function renderPostCard(p){
  ...
  el.querySelector(".sharePost")?.addEventListener("click", async (e)=>{
    ...
    // Toggle visibilità (pubblico/privato) robusto (desktop + mobile)
  el.querySelector(".toggle-vis-min")?.addEventListener("click", async (e)=>{
    const box = el.querySelector(".pensiero-box");
    if (!box) return;
    ...
    if (nextVis === "pubblico"){
      ...
      // se è la prima volta che diventa pubblico, proponi la riflessione veloce
...
async function renderList(fullReplace = true){
  ...
  for (const p of visiblePosts){
    ...
    try {
      ...
      if (emo) {
        ...
        attachEmotionActions(card.querySelector('.pensiero-box'), p.id);
  ...
  // ⬇️ Queste tre righe DEVONO stare fuori dal for
