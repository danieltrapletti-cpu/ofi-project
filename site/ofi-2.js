...
(function(){
  ...
  const chat = document.querySelector('#agentChat') || document.querySelector('.agent-chat');
  ...
  if (fab) fab.addEventListener('click', (e)=>{ e.preventDefault(); openPanel(); });
  ...
  function replyTo(q){
    ...
    const rules = [
      ...
      {k:['necrologio','necrologi','anniversario','anniversari'], a:`Dalla sezione <strong>Necrologi & Anniversari</strong> puoi pubblicare avvisi ufficiali. Gli avvisi sono visibili pubblicamente. <a href='faq.html#necrologi'>Guida</a>.`},
      ...
      {k:['luogo della memoria','memoria'], a:`Il <strong>Luogo della Memoria</strong> fissa sulla mappa un posto simbolico. Sarà visibile nel profilo e nella mappa. <a href='faq.html#ricordi'>Scopri di più</a>.`},
    ];
    const hit = rules.find(r => r.k.some(k => t.includes(k)));
    addOut(q);
    setTimeout(()=> addIn(hit ? hit.a : "Posso aprirti la guida o passarti ai contatti utili. Dimmi in che cosa ti aiuto."), 250);
  }
